...
const categories_flex_data = [
  ...
  categories_flex_data.forEach(({category, img})=>{
    ...
    const categoryElem = document.createElement('p') // параграф для текста, напр Kids

    categoryElem.innerText = category;
    cardElem.classList.add('category_card');
    imgElem.style.backgroundImage = `url('${img}')` // url это нестандартное подключение, можно еще просто через css. У картинки слой здесь всегда первы и мы на нем можем даже писать далее
    cardElem.append(imgElem, categoryElem)
    category_flex_block.append(cardElem) // отрисовали в нем теперь cardElem карточку
  })
...
const new_arrivals_container = document.querySelector('.new_arrivals_container'); // class
  ...
  new_arrivals_data.forEach(({img, title, price})=>{ // деструктуризация {}
    // создаем теги
    const cardElem = document.createElement('div'); // область видимости разная и др массив, поэтому мы можем использовать тоже название
    const imgElem = document.createElement('div');
    const titleElem = document.createElement('p'); // параграф для текста, напр title
    const priceElem = document.createElement('p');
    const heartElem = document.createElement('p');
    
    imgElem.style.backgroundImage = `url('${img}')` // url это нестандартное подключение, можно еще просто через css
    cardElem.classList.add('new_arrival_card');
    //heartElem.classList.add('fa-regular fa-heart heartElem'); // fa-regular с помощью библиотеки https://kit.fontawesome.com/d02d906550.js
    ...
    priceElem.innerText = `EUR ${price}` //`` так как надо указать единицу измерения, поэтому нужна шаблонная строка
    
    imgElem.append(heartElem) // heart положили в img. Также здесь находится backgroundImage
